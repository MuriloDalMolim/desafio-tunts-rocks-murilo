...
(async () => {
    ...
    for (let i = 0; i < 24; i++) {
        let miss = parseInt(rows[i].get("Faltas"));

        if (miss > 15) {
            sheet.getCell(i + 3, 6).value = "Reprovado por Falta";
            sheet.getCell(i + 3, 7).value = 0;
        } else {
            let first = parseInt(rows[i].get("P1"));
            let second = parseInt(rows[i].get("P2"));
            let third = parseInt(rows[i].get("P3"));
            let average = Math.ceil((first + second + third) / 3).toFixed(0);
